...
> - `agent-os orchestrator requirements-approval-execution-check <intake-id> --plan-id <plan-id> [PATH]` (read-only)
...
The decide-requirements-approval command records an owner-provided requirements approval decision for an existing `GOAL_INTAKE` and DRAFT planning workspace. It writes `.agent-os/orchestrator/intakes/<intake-id>/requirements-approval-decisions/<plan-id>/<decision-id>.json` with decision values `REQUEST_REQUIREMENTS_APPROVAL_REVISION`, `BLOCK_REQUIREMENTS_APPROVAL`, or `AUTHORIZE_REQUIREMENTS_APPROVAL`. The decision artifact is append-only and non-authoritative; it snapshots requirements approval preflight state at decision time for `AUTHORIZE_REQUIREMENTS_APPROVAL` only. `AUTHORIZE_REQUIREMENTS_APPROVAL` requires in-memory confirmation that requirements approval preflight would succeed; it does **not** persist a preflight artifact. It does **not** call an LLM, approve requirements, promote `DRAFT-REQ-*` to approved requirement IDs, assign `REQ-*` ids, generate user stories or acceptance criteria, generate architecture decisions, choose stack/database/networking/deployment, generate an implementation plan, generate `PLANNING_RUN_SLICE`, create approved requirements artifacts, validate or approve the workspace, transition workspace status, create runner proposals, create runs, or invoke an executor. It does **not** modify `local-agentic-spec.md`, validation reports, orchestrator intake artifacts, transport artifacts, provenance files, or other owner decision artifacts. **Requirements approval owner decision is not requirements approval and not promotion** — not architecture decision, not implementation planning, not validation or approval. `AUTHORIZE_REQUIREMENTS_APPROVAL` authorizes only a future separate requirements approval execution check; authorization is not approval and is not promotion. `REQUEST_REQUIREMENTS_APPROVAL_REVISION` and `BLOCK_REQUIREMENTS_APPROVAL` do not mutate the draft, validation report, or perform approval. Future requirements approval execution check, architecture decision, implementation plan, independent validation, and runner proposal generation remain future/separate work.

The requirements-approval-execution-check command performs a read-only pre-execution check before any future requirements approval command may run. It requires successful requirements approval preflight, a latest plan-scoped `AUTHORIZE_REQUIREMENTS_APPROVAL` owner decision whose preflight and validation-report metadata matches current state, and all validation report candidates still `PASS` with required non-promotion markers. It does **not** call an LLM, approve requirements, promote `DRAFT-REQ-*` to approved requirement IDs, assign `REQ-*` ids, rewrite `local-agentic-spec.md`, modify validation reports, generate user stories or acceptance criteria, generate architecture decisions, choose stack/database/networking/deployment, generate an implementation plan, generate `PLANNING_RUN_SLICE`, validate or approve the workspace, transition workspace status, create runner proposals, create runs, invoke an executor, or write any artifact. It does **not** modify orchestrator intake artifacts, transport artifacts, requirements draft provenance, validation reports, owner decision artifacts, `context-pack.md`, `local-agentic-spec.md`, `implementation-plan.md`, or `planning-audit.md`. **Requirements approval execution check is not requirements approval and not promotion** — owner authorization is not approval; validation report `PASS` is not approval; `DRAFT-REQ-*` remains draft-only. A later `REQUEST_REQUIREMENTS_APPROVAL_REVISION` or `BLOCK_REQUIREMENTS_APPROVAL` blocks approval even after an older `AUTHORIZE_REQUIREMENTS_APPROVAL`. Successful check means only that a separate future requirements approval command may be run separately. Future requirements approval, architecture decision, implementation plan, independent validation, and runner proposal generation remain future/separate work.

The create-approved-requirements command creates the first approved requirements artifact after successful requirements approval execution check (`REQUIREMENTS_APPROVAL_EXECUTION_CHECK_CONFIRMED_NO_APPROVAL_NO_PROMOTION`). It requires a DRAFT planning workspace with orchestrator provenance binding the plan to the intake, coherent validation report and draft state, and no existing `approved-requirements.json`. It writes `approved-requirements.json` at the planning workspace root with deterministic promotion mapping (`DRAFT-REQ-001` → `REQ-001`, etc.) inside the artifact only. It does **not** call an LLM, rewrite `local-agentic-spec.md`, modify validation reports, modify owner decision artifacts, generate user stories or acceptance criteria, generate architecture decisions, choose stack/database/networking/deployment, generate an implementation plan, generate `PLANNING_RUN_SLICE`, validate or approve the workspace, transition workspace status, create runner proposals, create runs, invoke an executor, or write any artifact other than `approved-requirements.json`. It does **not** modify orchestrator intake artifacts, transport artifacts, requirements draft provenance, `context-pack.md`, `implementation-plan.md`, or `planning-audit.md`. **Approved requirements artifact is not architecture, not implementation plan, not `PLANNING_RUN_SLICE`, and not runner authority** — `REQ-*` ids exist only inside `approved-requirements.json` at this stage; `DRAFT-REQ-*` remains in `local-agentic-spec.md`. Future architecture decision requires a separate command. Architecture decision, implementation plan, independent validation, and runner proposal generation remain future/separate work.

Requirements approval owner decision non-authority flags include (all `true`):

```json
{
  "owner_decision_is_not_approval": true,
  "authorizes_future_approval_only_when_decision_is_authorize": true,
  "approval_authorization_is_not_promotion": true,
  "validation_report_is_not_approval": true
}
```

The flag `authorizes_future_approval_only_when_decision_is_authorize` documents doctrine only: future approval authorization applies when `decision` is `AUTHORIZE_REQUIREMENTS_APPROVAL`. `REQUEST_REQUIREMENTS_APPROVAL_REVISION` and `BLOCK_REQUIREMENTS_APPROVAL` record `source_requirements_approval_preflight_next_action` as `NO_FUTURE_REQUIREMENTS_APPROVAL_AUTHORIZED_BY_THIS_DECISION`.
