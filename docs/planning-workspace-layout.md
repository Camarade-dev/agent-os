...
After a latest plan-scoped `AUTHORIZE_REQUIREMENTS_EXTRACTION` owner decision exists, run read-only requirements extraction execution check:

```bash
agent-os orchestrator requirements-extraction-execution-check <intake-id> --plan-id <plan-id> [PATH]
```

Requires an existing DRAFT scaffold with coherent requirements-extraction scaffold provenance, `local-agentic-spec.md` labeled `REQUIREMENTS_EXTRACTION_SCAFFOLD_NON_AUTHORITY` with only empty containers (`NO_REQUIREMENTS_EXTRACTED`, `NOT_GENERATED`, `UNDECIDED_NOT_GENERATED`), confirmed requirements-extraction preflight state, confirmed draft-preparation authorization, unmodified `implementation-plan.md` and `planning-audit.md` placeholders, and a latest plan-scoped requirements-extraction owner decision of `AUTHORIZE_REQUIREMENTS_EXTRACTION` with metadata matching current scaffold/provenance. **Does not write any artifact** — read-only pre-execution gate only. Execution check does **not** extract or infer requirements; does **not** generate requirements content or requirement IDs; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate any orchestrator, transport, provenance, decision, or planning artifact. **Successful execution check is not requirements extraction** — not requirements approval, not architecture decision, not implementation planning, not validation or approval. A later `REQUEST_MORE_CONTEXT` or `BLOCK_REQUIREMENTS_EXTRACTION` blocks extraction. Successful check means only that a separate future requirements extraction command may be run separately. Future requirements extraction, requirements validation, architecture decision, implementation plan, independent validation, and owner approval remain required. Runner proposal generation remains future/separate work.

After successful requirements-extraction execution check, write deterministic source-bounded requirements draft candidates:

```bash
agent-os orchestrator extract-requirements-draft <intake-id> --plan-id <plan-id> [PATH]
```

Requires an existing DRAFT scaffold with confirmed execution check state, latest plan-scoped `AUTHORIZE_REQUIREMENTS_EXTRACTION` owner decision, coherent requirements-extraction scaffold, `local-agentic-spec.md` labeled `REQUIREMENTS_EXTRACTION_SCAFFOLD_NON_AUTHORITY` with only empty containers, unmodified `implementation-plan.md` and `planning-audit.md` placeholders, present context transport JSON and `context-pack.md`, and no existing requirements draft provenance. Writes `REQUIREMENTS_DRAFT_NON_AUTHORITY` content to `local-agentic-spec.md` and provenance under `evidence/`:

- `evidence/orchestrator-requirements-draft-provenance.json` — links draft candidates to source transport, context-pack, scaffold provenance, owner decision, and execution check state; **provenance is traceability only, not authority**

The requirements draft provides deterministic `DRAFT-REQ-*` source-bounded candidates only; it does **not** approve or validate requirements; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate orchestrator intake artifacts, transport artifacts, context-pack draft provenance, requirements-extraction scaffold provenance, requirements-extraction owner decision artifacts, `context-pack.md`, `implementation-plan.md`, or `planning-audit.md`. **Requirements draft is not approved requirements** — not architecture decision, not implementation planning, not validation or approval. Broad goals remain broad unless source explicitly provides more detail. The planning workspace remains **`DRAFT`** — it is **not validated or approved**. Future requirements validation, architecture decision, implementation plan, independent validation, and owner approval remain required. Runner proposal generation remains future/separate work.

After a coherent requirements draft exists, run read-only requirements draft validation preflight:

```bash
agent-os orchestrator requirements-draft-validation-preflight <intake-id> --plan-id <plan-id> [PATH]
```

Requires an existing DRAFT scaffold with orchestrator provenance binding the plan to the intake; `local-agentic-spec.md` labeled `REQUIREMENTS_DRAFT_NON_AUTHORITY`; present `evidence/orchestrator-requirements-draft-provenance.json`; coherent source-bounded `DRAFT-REQ-*` candidates with required non-authority markers (`DRAFT_REQUIREMENT_CANDIDATE_NON_AUTHORITY`, `SOURCE_BOUNDED`, `NOT_VALIDATED`, `NOT_APPROVED`, `architecture_status: NOT_DECIDED`, `implementation_status: NOT_PLANNED`); provenance matching draft candidate count and IDs; no forbidden user stories, acceptance criteria, architecture decisions, implementation plan content, or `PLANNING_RUN_SLICE`; and a latest plan-scoped `AUTHORIZE_REQUIREMENTS_EXTRACTION` owner decision coherent with draft provenance. **Does not write any artifact** — read-only gate only. Validation preflight does **not** validate or approve requirements; does **not** rewrite or promote the draft; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate orchestrator intake artifacts, transport artifacts, provenance files, owner decision artifacts, `context-pack.md`, `local-agentic-spec.md`, `implementation-plan.md`, or `planning-audit.md`. **Successful preflight is not requirements validation** — not requirements approval, not architecture decision, not implementation planning. A later `REQUEST_MORE_CONTEXT` or `BLOCK_REQUIREMENTS_EXTRACTION` blocks validation preflight. Successful preflight means only that a separate future requirements validation command may be attempted. Future requirements validation, architecture decision, implementation plan, independent validation, and owner approval remain required. Runner proposal generation remains future/separate work.

After a requirements draft exists, record an owner requirements validation decision:
...
Requires an existing DRAFT scaffold with orchestrator provenance binding the plan to the intake. `AUTHORIZE_REQUIREMENTS_VALIDATION` additionally requires in-memory confirmation that requirements draft validation preflight would succeed; it does not persist a preflight artifact. Writes an append-only decision artifact under `.agent-os/orchestrator/intakes/<intake-id>/requirements-validation-decisions/<plan-id>/<decision-id>.json`. **Does not validate or approve requirements**; does **not** promote `DRAFT-REQ-*` to approved requirement IDs; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** write validation reports; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate `local-agentic-spec.md`, orchestrator intake artifacts, transport artifacts, provenance files, or other owner decision artifacts. **Requirements validation owner decision is not requirements validation** — not requirements approval, not architecture decision, not implementation planning, not validation or approval. `AUTHORIZE_REQUIREMENTS_VALIDATION` authorizes only a future separate requirements validation command; authorization is not validation and is not approval. `REQUEST_REQUIREMENTS_DRAFT_REVISION` and `BLOCK_REQUIREMENTS_VALIDATION` do not mutate the draft or perform validation. Future requirements validation, architecture decision, implementation plan, independent validation, and owner approval remain required. Runner proposal generation remains future/separate work.

After a latest plan-scoped `AUTHORIZE_REQUIREMENTS_VALIDATION` owner decision exists, run read-only requirements validation execution check:

```bash
agent-os orchestrator requirements-validation-execution-check <intake-id> --plan-id <plan-id> [PATH]
```

Requires a DRAFT planning workspace with orchestrator provenance binding the plan to the intake; coherent `REQUIREMENTS_DRAFT_NON_AUTHORITY` draft and provenance; confirmed requirements draft validation preflight state; and a latest plan-scoped requirements-validation owner decision of `AUTHORIZE_REQUIREMENTS_VALIDATION` whose preflight/draft snapshot metadata matches current state. **Does not write any artifact** — read-only gate only. Execution check does **not** validate or approve requirements; does **not** promote `DRAFT-REQ-*` to approved requirement IDs; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** write validation reports; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate orchestrator intake artifacts, transport artifacts, provenance files, owner decision artifacts, `context-pack.md`, `local-agentic-spec.md`, `implementation-plan.md`, or `planning-audit.md`. **Successful execution check is not requirements validation** — not requirements approval, not architecture decision, not implementation planning, not validation or approval. Owner authorization is not validation and is not approval. A later `REQUEST_REQUIREMENTS_DRAFT_REVISION` or `BLOCK_REQUIREMENTS_VALIDATION` blocks validation even after an older `AUTHORIZE_REQUIREMENTS_VALIDATION`. Successful check means only that a separate future requirements validation command may be run separately. Future requirements validation, architecture decision, implementation plan, independent validation, and owner approval remain required. Runner proposal generation remains future/separate work.

After successful requirements validation execution check, write deterministic requirements draft validation report:

```bash
agent-os orchestrator validate-requirements-draft <intake-id> --plan-id <plan-id> [PATH]
```

Requires successful requirements validation execution check (`REQUIREMENTS_VALIDATION_EXECUTION_CHECK_CONFIRMED_NO_VALIDATION_PERFORMED`); a latest plan-scoped `AUTHORIZE_REQUIREMENTS_VALIDATION` owner decision; coherent `REQUIREMENTS_DRAFT_NON_AUTHORITY` draft and provenance; and no existing validation report. Writes `evidence/orchestrator-requirements-draft-validation-report.json` with per-candidate validation results (`PASS`, `NEEDS_REVISION`, or `BLOCKED`) and required non-authority markers (`approval_status: NOT_APPROVED`, `promotion_status: NOT_PROMOTED`, `approved_requirement_id: NOT_ASSIGNED`). **Does not approve requirements**; does **not** promote `DRAFT-REQ-*` to approved requirement IDs; does **not** rewrite `local-agentic-spec.md`; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate orchestrator intake artifacts, transport artifacts, requirements draft provenance, owner decision artifacts, `context-pack.md`, `local-agentic-spec.md`, `implementation-plan.md`, or `planning-audit.md`. **Validation report is not approval and not promotion** — `PASS` does not approve requirements and does not assign `REQ-*` ids; `DRAFT-REQ-*` remains draft-only. Future requirements approval requires a separate owner decision. Architecture decision, implementation plan, independent validation, and runner proposal generation remain future/separate work.

After a requirements draft validation report exists with all candidates `PASS`, run read-only requirements approval preflight:

```bash
agent-os orchestrator requirements-approval-preflight <intake-id> --plan-id <plan-id> [PATH]
```

Requires successful requirements validation execution check; an existing validation report with status `REQUIREMENTS_DRAFT_VALIDATION_REPORT_CREATED_NO_APPROVAL_NO_PROMOTION` and next action `FUTURE_REQUIREMENTS_APPROVAL_REQUIRES_OWNER_DECISION`; plan/intake scope match; coherent validation report non-authority flags; candidate coverage matching the current draft; and all candidate validation results `PASS` with `approval_status: NOT_APPROVED`, `promotion_status: NOT_PROMOTED`, and `approved_requirement_id: NOT_ASSIGNED`. **Does not write any artifact** — read-only gate only. Approval preflight does **not** record owner decisions; does **not** approve requirements; does **not** promote `DRAFT-REQ-*` to approved requirement IDs; does **not** rewrite `local-agentic-spec.md`; does **not** modify validation reports; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate orchestrator intake artifacts, transport artifacts, requirements draft provenance, validation reports, owner decision artifacts, `context-pack.md`, `local-agentic-spec.md`, `implementation-plan.md`, or `planning-audit.md`. **Requirements approval preflight is not owner approval and not promotion** — validation report `PASS` is not approval; `DRAFT-REQ-*` remains draft-only. Successful preflight means only that a separate future requirements approval owner decision command may be attempted. Future requirements approval, architecture decision, implementation plan, independent validation, and runner proposal generation remain future/separate work.

After a requirements draft validation report exists, record an owner requirements approval decision:

```bash
agent-os orchestrator decide-requirements-approval <intake-id> --plan-id <plan-id> --decision <REQUEST_REQUIREMENTS_APPROVAL_REVISION|BLOCK_REQUIREMENTS_APPROVAL|AUTHORIZE_REQUIREMENTS_APPROVAL> --decision-id <decision-id> --summary "<owner summary>" [PATH]
```

Requires an existing DRAFT scaffold with orchestrator provenance binding the plan to the intake. `AUTHORIZE_REQUIREMENTS_APPROVAL` additionally requires in-memory confirmation that requirements approval preflight would succeed; it does not persist a preflight artifact. Writes an append-only decision artifact under `.agent-os/orchestrator/intakes/<intake-id>/requirements-approval-decisions/<plan-id>/<decision-id>.json`. **Does not approve requirements**; does **not** promote `DRAFT-REQ-*` to approved requirement IDs; does **not** assign `REQ-*` ids; does **not** generate user stories or acceptance criteria; does **not** generate architecture decisions, implementation plans, or `PLANNING_RUN_SLICE`; does **not** create approved requirements artifacts; does **not** modify validation reports; does **not** validate or approve the workspace; does **not** transition status; does **not** create runner proposals, runs, or executor invocations; and does **not** mutate `local-agentic-spec.md`, orchestrator intake artifacts, transport artifacts, provenance files, or other owner decision artifacts. **Requirements approval owner decision is not requirements approval and not promotion** — not architecture decision, not implementation planning, not validation or approval. `AUTHORIZE_REQUIREMENTS_APPROVAL` authorizes only a future separate requirements approval execution check; authorization is not approval and is not promotion. Future requirements approval execution check, architecture decision, implementation plan, independent validation, and runner proposal generation remain future/separate work.

After `AUTHORIZE_REQUIREMENTS_APPROVAL`, run a read-only requirements approval execution check:

```bash
agent-os orchestrator requirements-approval-execution-check <intake-id> --plan-id <plan-id> [PATH]
```
