...
class M1ArtifactTests(unittest.TestCase):
    def test_m1_json_artifacts_are_canonical_and_match_code_catalog(self) -> None:
        ...
        second_repair_report_path = ROOT / "implementation" / "M1_SECOND_BOUNDED_REPAIR_REPORT.json"
        ...
        second_repair_report = parse_canonical_json(
            second_repair_report_path.read_bytes(), label=str(second_repair_report_path)
        )
        ...
        self.assertEqual(canonical_bytes(second_repair_report), second_repair_report_path.read_bytes())
        ...
        self.assertEqual(len(catalog["schemas"]), 29)
        self.assertEqual(report["schema_count"], 29)
        ...
        self.assertEqual(second_repair_report["schema_count"], 29)
        self.assertEqual(second_repair_report["previous_schema_count"], 27)
        self.assertEqual(second_repair_report["new_schema_count"], 2)
        self.assertEqual(
            [finding["id"] for finding in second_repair_report["repair_findings"]],
            ["M1-R06", "M1-R07", "M1-R08", "M1-R09", "M1-R10", "M1-R11"],
        )
        for finding in second_repair_report["repair_findings"]:
            self.assertEqual(finding["status"], "CLOSED")
            self.assertTrue(finding["false_positive_reproduction"])
        self.assertEqual(second_repair_report["terminal_verdict"], "M1_SECOND_BOUNDED_REPAIRS_VERIFIED")
        self.assertEqual(second_repair_report["starting_commit"], "41942a3ed3a85d4f47b38a29b9d86368523555cd")
        self.assertFalse(second_repair_report["boundary_audit"]["milestone_2_started"])
        ...
        for descriptor in tool_schemas:
            ...
            self.assertTrue(descriptor["exact_request_binding"])
        grammar_schemas = [
            item for item in catalog["schemas"] if item["schema_id"].endswith("tool_grammar_specification")
            or item["schema_id"].endswith("tool_grammar_entry")
        ]
        self.assertEqual(len(grammar_schemas), 2)
